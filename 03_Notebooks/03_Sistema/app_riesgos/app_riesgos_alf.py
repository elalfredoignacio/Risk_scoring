...
with st.sidebar:
    ...
    principal = st.number_input('Importe Solicitado', 500, 50000) # por defecto 500, máximo 50000
...
#CALCULAR RIESGO
# Lo meto en un if para que solamente lo ejecute cuando se apriete el botón "CALCULAR RIESGO"
if st.sidebar.button('CALCULAR RIESGO'):
    #Ejecutar el scoring
    EL = ejecutar_modelos(registro)

    #Calcular los kpis 
    kpi_pd = round((EL.pd * 100), 2)
    kpi_ead = round((EL.ead * 100), 2)
    kpi_lgd = round((EL.lgd * 100), 2)
    ...
    # lo pongo con with al col1 porque sino el st_echarts no funciona
    with col1:
